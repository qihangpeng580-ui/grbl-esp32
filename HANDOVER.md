...
| 文件 | 说明 |
|------|------|
| `Grbl_Esp32/src/PaperFeed.h` | 进纸电机头文件，参数宏定义 |
| `Grbl_Esp32/src/PaperFeed.cpp` | 阻塞式步进脉冲生成，直接 GPIO 控制 |
| `Grbl_Esp32/src/Machines/HE_3AXIS.h` | 引脚定义（末尾新增） |

修改文件：`src/Grbl.h`、`src/GCode.h`、`src/GCode.cpp`、`src/Grbl.cpp`（均用 `#ifdef PAPER_FEED_STEP_PIN` 条件编译包裹）。
...
| 进纸 STEP | 13 |
| 进纸 DIR | 15 |
| 使能 | 25（与轴共用 STEPPERS_DISABLE_PIN） |
...
```
上位机发 M100 P10 F500 → 串口解析 → sync Planner → motors_set_disable(false)
  → set_dir → for N 步: pulse STEP pin → motors_set_disable(true) → OK
```
