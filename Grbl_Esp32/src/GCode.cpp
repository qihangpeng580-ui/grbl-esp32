...
Error gc_execute_line(char* line, uint8_t client) {
           ...
           NOTE: Modal group numbers are defined in Table 4 of NIST RS274-NGC v3, pg.20 */
            case 'G':
                ...
                switch (int_value) {
                    ...
                    case 61:
                        ...
                        break;
                    default:
                        // Add support for runtime kinematics switching: G800 = CoreXY, G801 = Cartesian, G802 = SwapXY
                        if (int_value == 800) {
                            gc_block.non_modal_command = NonModal::KinematicsCoreXY;
                            mg_word_bit                 = ModalGroup::MG0;
                        } else if (int_value == 801) {
                            gc_block.non_modal_command = NonModal::KinematicsCartesian;
                            mg_word_bit                 = ModalGroup::MG0;
                        } else if (int_value == 802) {
                            gc_block.non_modal_command = NonModal::KinematicsSwapXY;
                            mg_word_bit                 = ModalGroup::MG0;
                        } else {
                            FAIL(Error::GcodeUnsupportedCommand);  // [Unsupported G command]
                        }
            ...
            case 'M':
                ...
                switch (int_value) {
                    ...
                    case 100:  // M100 - Paper feed forward
                    case 101:  // M101 - Paper feed backward
                    case 102:  // M102 - Stop paper feed
                        gc_block.paper_feed_cmd = (uint8_t)int_value;
                        mg_word_bit = ModalGroup::MM11;
                        break;
#endif
#ifdef SERVO_PIN
                    case 103:  // M103 - Servo set angle
    ...
    */
    ...
    // [11. Set active plane ]
#ifdef PAPER_FEED_STEP_PIN
    if (gc_block.paper_feed_cmd == 100 || gc_block.paper_feed_cmd == 101) {
        if (bit_isfalse(value_words, bit(GCodeWord::P))) {
            FAIL(Error::GcodeValueWordMissing);
        }
        bit_false(value_words, bit(GCodeWord::P));
    ...
    switch (gc_block.non_modal_command) {
        case NonModal::SetCoordinateData:
            ...
            break;
        case NonModal::KinematicsCoreXY:
            kinematics_set_mode(KINEMATICS_COREXY);
            grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "Kinematics set to CoreXY");
            break;
        case NonModal::KinematicsCartesian:
            kinematics_set_mode(KINEMATICS_CARTESIAN);
            grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "Kinematics set to Cartesian");
            break;
        case NonModal::KinematicsSwapXY:
            kinematics_set_mode(KINEMATICS_SWAPXY);
            grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "Kinematics set to SwapXY (X<->Y)");
            break;
    ...
    */
    ...
    // [9. Override control ]
#ifdef PAPER_FEED_STEP_PIN
    if (gc_block.paper_feed_cmd == 100) {
        paper_feed_sync(gc_block.values.p, gc_block.values.f, true);
    }
    if (gc_block.paper_feed_cmd == 101) {
        paper_feed_sync(gc_block.values.p, gc_block.values.f, false);
    }
    if (gc_block.paper_feed_cmd == 102) {
        paper_feed_stop();
    }
#endif
#ifdef SERVO_PIN
