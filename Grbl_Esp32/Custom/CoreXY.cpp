...
static void cartesian_to_motors(float* position) {
    ...
    position[X_AXIS] = motors[Y_AXIS];
    position[Y_AXIS] = motors[X_AXIS];
...
bool user_defined_homing(uint8_t cycle_mask) {
        ...
        for (uint8_t axis = X_AXIS; axis <= n_axis; axis++) {
            if (bit(axis) == mask) {
                ...
                do {
                    ...
                    bool dirMask = bit_istrue(homing_dir_mask->get(), bit(axis));
#ifdef Y_HOMING_INVERT
                    if (axis == Y_AXIS) dirMask = !dirMask;
#endif
                    if (dirMask) {
    ...
    // Output current position after CoreXY homing completes
    grbl_sendf(CLIENT_SERIAL, "[HOMED] X=%.3f Y=%.3f Z=%.3f\r\n",
        system_get_mpos()[X_AXIS], system_get_mpos()[Y_AXIS], system_get_mpos()[Z_AXIS]);

    return true;
}

// Y电机取反，匹配 cartesian_to_motors
//   motors[X_AXIS] = X - Y
//   motors[Y_AXIS] = -(X + Y)
...
bool cartesian_to_motors(float* target, plan_line_data_t* pl_data, float* position) {
    // Support three modes: Cartesian (0), CoreXY (1), SwapXY (2)
    kinematics_mode_t mode = kinematics_get_mode();

    float target_copy[MAX_N_AXIS];
    ...
    if (mode == KINEMATICS_SWAPXY) {
        // Swap X and Y in local copies
        float tx = target_copy[X_AXIS];
        target_copy[X_AXIS] = target_copy[Y_AXIS];
        target_copy[Y_AXIS] = tx;
        float px = position_copy[X_AXIS];
        position_copy[X_AXIS] = position_copy[Y_AXIS];
        position_copy[Y_AXIS] = px;
    }

    // Cartesian mode: just pass through to mc_line
    if (mode == KINEMATICS_CARTESIAN) {
        return mc_line(target_copy, pl_data);
    }
    ...
    float motors[n_axis];
    transform_cartesian_to_motors(motors, target_copy);

    if (!pl_data->motion.rapidMotion) {
        float last_motors[n_axis];
        transform_cartesian_to_motors(last_motors, position_copy);
        pl_data->feed_rate *= (three_axis_dist(motors, last_motors) / dist);
    }

    // If mode is SwapXY with CoreXY semantics, undo swap on motors mapping:
    if (mode == KINEMATICS_SWAPXY) {
        // For swap mode combined with CoreXY, swap motor outputs X<->Y so machine axes are swapped
        float swapped_motors[MAX_N_AXIS];
        for (int i = 0; i < n_axis; i++) swapped_motors[i] = motors[i];
        float tmp = swapped_motors[X_AXIS];
        swapped_motors[X_AXIS] = swapped_motors[Y_AXIS];
        swapped_motors[Y_AXIS] = tmp;
        return mc_line(swapped_motors, pl_data);
    }

    return mc_line(motors, pl_data);
}

// motors -> cartesian：根据当前模式做逆向解算
//
//   Cartesian(0): 直接复制
//   CoreXY(1):    正向 mA=X-Y, mB=-(X+Y)  → 逆向 X=0.5*(mA-mB)/gf, Y=-0.5*(mA+mB)
//   SwapXY(2):    正向 交换XY→CoreXY→交换电机  → 逆向 交换电机→CoreXY逆→交换XY
void motors_to_cartesian(float* cartesian, float* motors, int n_axis) {
    ...
    float mA = motors[X_AXIS];
    float mB = motors[Y_AXIS];

    if (mode == KINEMATICS_SWAPXY) {
        // SwapXY: 先交换电机输入（恢复正向中交换电机输出的影响）
        float tmp = mA;
        mA = mB;
        mB = tmp;
    }

    // CoreXY 逆向解算（匹配新正向公式 mA=X-Y, mB=-(X+Y)）
    cartesian[X_AXIS] = 0.5 * (mA - mB) / geometry_factor;
    cartesian[Y_AXIS] = -0.5 * (mA + mB);

    if (mode == KINEMATICS_SWAPXY) {
        // SwapXY: 再交换XY输出（恢复正向中交换目标XY的影响）
        float tmp = cartesian[X_AXIS];
        cartesian[X_AXIS] = cartesian[Y_AXIS];
        cartesian[Y_AXIS] = tmp;
    }
