...
?????PowerShell??
```powershell
$d = "code_backups/$(Get-Date -Format 'yyyy-MM-dd')"
New-Item -ItemType Directory -Path $d -Force
Copy-Item Grbl_Esp32/src/ServoControl.cpp, Grbl_Esp32/src/PaperFeed.cpp, ... -Destination $d
```

### 15.2 v1.3 ?????2026-07-23?

| ?? | ?? |
|------|------|
| `Grbl_Esp32/src/ServoControl.cpp` | ???? v2 FreeRTOS?`srv_moving=true` ??? `xQueueSend` ??`servo_task` ??????????`servo_set_angle` ?? `protocol_buffer_synchronize()`??? 120s |
| `Grbl_Esp32/src/PaperFeed.cpp` | ???? v2 FreeRTOS?`pf_task_done=true` ?? `pf_moving=false`??????? `protocol_execute_realtime()` ????? |
| `Grbl_Esp32/src/MotionControl.cpp` | CoreXY ????? `grbl_msg_sendf` ?? `[HOMED] X=... Y=... Z=...` |
| `Grbl_Esp32/src/Limits.cpp` | ??????????????Z??? CoreXY ?? |
| `Grbl_Esp32/Custom/CoreXY.cpp` | `user_defined_homing` ? Y ??????? `Y_HOMING_INVERT` ?? |
| `Grbl_Esp32/src/Machines/HE_3AXIS.h` | ?? `#define Y_HOMING_INVERT 1` |

### 15.3 ?????????

1. **GPIO2 ????**?Z_LIMIT_PIN ?? GPIO2?ESP32 strapping ???????????
2. **5V ????**????? VCC ??? 3.3V?5V ?? ESP32 GPIO
3. **CoreXY ????**?`$Homing/Cycle` ???????????? XY ???
