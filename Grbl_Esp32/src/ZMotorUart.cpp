...
namespace ZMotor {
bool read_status(uint8_t& status){const uint8_t q[]={ADDR,0x3A,0x6B};ZMotorUart::flush_input();if(ZMotorUart::write(q,sizeof(q))!=sizeof(q))return false;return read_frame(0x3A,&status,1,200);}
bool raw_hex(const char* text,uint8_t* response,size_t& response_length){if(!text||!response)return false;uint8_t frame[32]={};size_t n=0;const char*p=text;while(*p&&n<sizeof(frame)){while(*p==' '||*p==','||*p=='\t')p++;if(!*p)break;char*end=nullptr;unsigned long v=strtoul(p,&end,16);if(end==p||v>255)return false;frame[n++]=uint8_t(v);p=end;}if(!n)return false;ZMotorUart::flush_input();if(ZMotorUart::write(frame,n)!=n)return false;response_length=0;uint32_t start=millis();while(millis()-start<300){if(!ZMotorUart::available()){delay(1);continue;}int v=ZMotorUart::read();if(v>=0&&response_length<32)response[response_length++]=uint8_t(v);if(response_length&&response[response_length-1]==0x6B)return true;}return false;}
