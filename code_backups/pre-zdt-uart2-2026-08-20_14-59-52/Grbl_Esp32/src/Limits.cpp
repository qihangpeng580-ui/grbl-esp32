...
void limits_go_home(uint8_t cycle_mask) {
    ...
    // Output current position after homing completes
    float homed_mpos[MAX_N_AXIS];
    system_convert_array_steps_to_mpos(homed_mpos, sys_position);
    grbl_msg_sendf(CLIENT_ALL, MsgLevel::Info,
        "[HOMED] X=%.3f Y=%.3f Z=%.3f",
        homed_mpos[X_AXIS], homed_mpos[Y_AXIS], homed_mpos[Z_AXIS]);
}
